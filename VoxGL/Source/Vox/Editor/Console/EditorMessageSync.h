...
#include "../../VoxGL-Editor/Source/Panels/EditorConsolePanel.h"
#include "spdlog/sinks/base_sink.h"
#include <mutex>

namespace Vox
{
	class EditorConsoleSync : public spdlog::sinks::base_sink<std::mutex>
	{
	public:
		explicit EditorConsoleSync(uint32_t bufferCapacity)
			: m_MessageBufferCapacity(bufferCapacity), m_MessageBuffer(bufferCapacity) {}

		virtual ~EditorConsoleSync() = default;

		EditorConsoleSync(const EditorConsoleSync& other) = delete;
		EditorConsoleSync& operator=(const EditorConsoleSync& other) = delete;

	protected:
		void sink_it_(const spdlog::details::log_msg& msg) override
		{
			fmt::v5::memory_buffer formatted;
			...
			{
				shortMessage, longMessage, GetMessageFlags(msg.level),
