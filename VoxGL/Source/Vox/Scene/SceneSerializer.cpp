...
namespace YAML 
{
	...
	template<>
	struct convert<Vox::UUID>
	{
		static Node encode(const Vox::UUID& uuid)
		{
			Node node;
		...
		static bool decode(const Node& node, Vox::UUID& uuid)
		{
...
namespace Vox 
{
	...
	bool SceneSerializer::Deserialize(const std::string& filepath)
	{
		...
		{
			LOG_CORE_ERROR("Failed to load .scene file '{0}'\n {1}", filepath, e.what());
			return false;
		}
